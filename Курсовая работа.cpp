...
void read_file() {
    ifstream file("Katalog.txt"); // открываем файл на чтение
    ...
    q = 6;
    for (int i = 0; i < q; i++) { // читаем информацию о товарах
        file >> kolvo[i].nomer >> kolvo[i].name >> kolvo[i].kolvo_tovarov_odnogo_tipa >> kolvo[i].cena_za_tovar >> kolvo[i].weight;
    }
...
void read() {
    ...
    int count;
    file >> count; // считываем количество товаров
    printf("\n");
    printf("№          Назв. товара        Кол-во          Стоимость      Вес    \n");
    printf("---------------------------------------------------------------------\n");
    for (int i = 0; i < count; i++) { // читаем информацию о товарах и выводим в таблицу
        int nomer, kolvo_tovarov_odnogo_tipa, cena_za_tovar, weight;
        string name;
        file >> nomer >> name >> kolvo_tovarov_odnogo_tipa >> cena_za_tovar >> weight;
        printf("%-10d %-20s %-15d %-15d %-15d ", nomer, name.c_str(), kolvo_tovarov_odnogo_tipa, cena_za_tovar, weight);
        printf("\n");
        printf("-------------------------------------------------------------------\n");
    }
    file.close(); // закрываем файл
...
int user()
{
    ...
    int choice = 0;
    do {
        cout << "1 - Просмотреть новый каталог товаров(созданный администратором)\n2 - Отсортировать по возрвстнаию номер информации\n3 - Просмотр старого каталога \n4 - Выйти из программы";
        cin >> choice;
        switch (choice)
        {
        case 1:view(); break;
